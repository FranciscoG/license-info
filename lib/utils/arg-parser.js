 ...
 * @throws Will exit the process with an error message if an unknown argument is 
 * encountered or if a value is missing for an argument that expects one.
 */
export function parseArgs(args, argSchemas) {
	...
	while (i < args.length) {
		...
		if (arg === cleanedArg && (i + 1 >= args.length || argSet.has(cleanArg(args[i + 1])))) {
			// if there is no next argument or the next argument is another flag, then we consider it a missing value
