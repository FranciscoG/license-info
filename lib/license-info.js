...
export class LicenseInfo {
   ...
   * entry whose `trees` field records every path that reaches it.
   *
   * @param {object} deps - Dependencies object
   * @param {string[]} parent - Parent dependency tree
   * @param {Record<string, PackageInfo>} [licenses] - Accumulator for deduped packages
   * @param {{[key: string]: number}} [licenseCount] - License count accumulator
   * @returns {{licenses: Record<string, PackageInfo>, licenseCount: Record<string, number>}}
   */
  getFlattenedDependencies(deps = {}, parent = [], licenses = {}, licenseCount = {}) {
    ...
    for (const [name, info] of Object.entries(deps)) {
      ...
      if (info.dependencies) {
        this.getFlattenedDependencies(info.dependencies, tree, licenses, licenseCount);
      }
    }
    return { licenses, licenseCount };
  }
