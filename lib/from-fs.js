...
/**
 * @typedef {Array<{name: string, version: string, license: string}>} PackageInfo
 */
 ...
 * @param {Set<string>} [packagesFound=new Set()] - A set to keep track of found packages
 * @returns {PackageInfo} - An array of package info
 */
export function getAllPackagesFromFS(dir, visited = new Set(), packagesFound = new Set()) {
  /**
   * @type {PackageInfo}
   */
  const packages = [];
    ...
    if (name.startsWith("@")) {
      ...
      for (const child of scopeChildren) {
        ...
        try {
          ...
          const pkgVer = `${pkgInfo.name}@${pkgInfo.version}`;
          if (!packagesFound.has(pkgVer)) {
            packagesFound.add(pkgVer);
            packages.push(pkgInfo);
          }

          // Recurse into nested node_modules inside this package
          packages.push(...getAllPackagesFromFS(real, visited, packagesFound));
        } catch {
